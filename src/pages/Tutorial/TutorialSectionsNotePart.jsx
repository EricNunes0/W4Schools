...
export default function TutorialSectionsNotePart(codes) {
    ...
    for(const code of codes) {
        switch(code.type) {
            ...
            case "text":
                ...
                parts.push(
                    <p className = "tutorial-sections-notes-paragraphs" data-bold = {bold}>{code.text}</p>
                );
                break;
            case "link":
                parts.push(
                    <a href = {code.link} target = {code.target || "_self"} className = "tutorial-sections-notes-links">{code.text}</a>
                );
                break;
        };
    };
    ...
    return (
        <div className = "tutorial-sections-notes-subparts">
            {parts}
        </div>
    );
};