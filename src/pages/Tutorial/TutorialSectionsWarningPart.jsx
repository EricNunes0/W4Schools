...
export default function TutorialSectionsNotePart(codes) {
    let parts = [];
    for(const code of codes) {
        switch(code.type) {
            ...
            case "text":
                ...
                parts.push(
                    <p className = "tutorial-sections-warnings-paragraphs" data-bold = {bold}>{code.text}</p>
                );
                break;
            case "link":
                parts.push(
                    <a href = {code.link} target = {code.target || "_self"} className = "tutorial-sections-warnings-links">{code.text}</a>
                );
                break;
        };
    };
    ...
    return (
        <div className = "tutorial-sections-warnings-subparts">
            {parts}
        </div>
    );
};